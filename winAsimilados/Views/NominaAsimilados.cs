...
namespace winAsimilados.Views
{
    public partial class NominaAsimilados : Form
    {
        ...
        E.Parametros parametrosLayoutBanorte = new Parametros();
        E.Empleado empleadoLayoutBanorte = new Empleado();
        E.LayoutBanorte layoutBanorte = new LayoutBanorte();
            ...
            try
            {
                ...
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (rCnt = 2; rCnt <= rw; rCnt++)
                    {
                        ...
                        decimal importe = Math.Round(Convert.ToDecimal((range.Cells[rCnt, "B"] as Excel.Range).Value2.ToString()), 2);
                        string periodicidad = (range.Cells[rCnt, "C"] as Excel.Range).Value2.ToString();
                        periodicidad = periodicidad.ToUpper();
                        ...
                        if (periodicidad.Equals("SEMANAL"))
                        {
                            ...
                            worksheetSemanal.Cells[6, 3] = importe;
                        ...
                        else if (periodicidad.Equals("QUINCENAL"))
                        {
                            ...
                            worksheetQuincenal.Cells[6, 3] = importe;
        ...
        {
            //this.CalcExcel("05",bd);  
            try
            {
                this.GenCarExcel();
                //this.LeerExcel(Convert.ToDecimal(10000.00),calcExcel);
            }
            catch (Exception Excel)
            {
                XtraMessageBox.Show(Excel.Message + "\nLeerExcel()", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void BtnGenCaratula_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int cont = 0;
                parametrosLayoutBanorte = controlador.GetParametros(rfc);
                var listaLayoutBanorte = new List<E.LayoutBanorte>();
                var empleadoMasivoLayoutBanorte = new List<E.Empleado>();
                E.Empleado[] emplMasiv = null;
                for (int i = 0; i < gridViewLayoutBanorte.RowCount; i++)
                {
                    if (gridViewLayoutBanorte.IsRowSelected(i))
                    {
                       ...
                       empleadoLayoutBanorte = controlador.BuscaEmpleado(rfcEmplMasiv);

                        if (empleadoLayoutBanorte.cuenta.Equals("No Definido") && empleadoLayoutBanorte.clabe_bancaria.Equals("No Definido"))
                        {
                            cuentaDestino = "No Definido";
                        }

                        if (empleadoLayoutBanorte.cuenta.Equals("0") && empleadoLayoutBanorte.clabe_bancaria.Equals("0"))
                        {
                            cuentaDestino = "No Definido";
                        }

                        if (empleadoLayoutBanorte.clabe_bancaria.Equals("No Definido") || empleadoLayoutBanorte.clabe_bancaria.Equals("0"))
                        {
                            cuentaDestino = empleadoLayoutBanorte.cuenta;
                        }
                        else
                        {
                            cuentaDestino = empleadoLayoutBanorte.clabe_bancaria;
                        }
                        
                        if (empleadoLayoutBanorte.cuenta.Equals("No Definido")|| empleadoLayoutBanorte.cuenta.Equals("0"))
                        {
                            cuentaDestino = empleadoLayoutBanorte.clabe_bancaria;
                        }
                        else
                        ...
                        listaLayoutBanorte.Add(new E.LayoutBanorte
                        {
                            numEmpl = Convert.ToInt32(numEmplLayoutBanorte),
                            nombreEmpleado = empleadoLayoutBanorte.Nombre,
                            RFCEmpleado = empleadoLayoutBanorte.RFC,
                            CURPEmpleado = empleadoLayoutBanorte.CURP,
                            ...
                            cuentaOrigen = parametrosLayoutBanorte.cuentaOrigen,
                            cuentaClaveDestino = cuentaDestino,
                            importe = netoMasiv,
                            referencia = numEmplLayoutBanorte + cont,
                            descripcion = descPago,
                            RFCOrdenante = parametrosLayoutBanorte.RFC,
                            IVA = 0,
                            ...
                            institucionPago = empleadoLayoutBanorte.Nombre,
                            claveTipoCambio = "0",
                            fechaCreacion = System.DateTime.Today,
                            banco = empleadoLayoutBanorte.banco,
                            caratula = nombreCaratula
                        });

                        ////XtraMessageBox.Show("Fin instrucciones");
                        empleadoMasivoLayoutBanorte.Add(new E.Empleado
                        {
                            RFC = rfcEmplMasiv,
                            IngresosBrutos = ingresosMasiv,
                            ISR = ISRMasiv,
                        ...
                        emplMasiv = empleadoMasivoLayoutBanorte.ToArray();
                    }

                }

                if (listaLayoutBanorte.Count.Equals(0))
                {
                    XtraMessageBox.Show("Por favor, Seleccione una celda.", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    controlador.InsertaLayout(listaLayoutBanorte);
                    controlador.GeneraLayoutBanorte(listaLayoutBanorte);//, parametrosLayoutBanorte);
                    //if (ValidaCer().Equals(false))
                    //{
                    //    return;
                    //}
                    ...
                    //DateTime FechaFinMasivo = Convert.ToDateTime(FecFinPeriMasiv.EditValue.ToString());
                    //DateTime FechaPago = Convert.ToDateTime(FecPagoMasiv.Text);
                    //controlador.GenXmlMasivo(empleadoMasivoLayoutBanorte, splash, empresa, rfc, ip, FechaInicioMasivo, FechaFinMasivo, FechaPago);
                }
            }
